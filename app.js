// Initialize variables
        ...
        function trackBus() {
            ...
            // Center map on the selected bus if it's active
            if (busData[busId] && busData[busId].status === "active") {
                map.setView([busData[busId].lat, busData[busId].lng], 14);
                showToast(`Now tracking Bus ${busId}`);
                
                // Create a special marker for the tracked bus
                if (trackedBusMarker) {
                    map.removeLayer(trackedBusMarker);
                }
                
                trackedBusMarker = L.marker([busData[busId].lat, busData[busId].lng], {
                    icon: L.divIcon({
                ...
                trackedBusMarker.bindPopup(`<b>Tracked Bus ${busId}</b><br>${busData[busId].name}`).openPopup();
            } else {
                showToast(`Bus ${busId} is not currently active`);
            }
        }
        ...
        function useFallbackData() {
            // Update fallback data with some randomization to simulate movement
            for (const busId in fallbackBusData) {
                if (busActivityStatus[busId]) {
                    // If this bus is being actively tracked, use the real location
                    // (this will be updated by the GPS watch function)
                    continue;
                }
                ...
                busData[busId] = {
                    ...fallbackBusData[busId],
                    lat: fallbackBusData[busId].lat + latChange,
                    lng: fallbackBusData[busId].lng + lngChange,
                    lastUpdate: new Date()
                };
                
                // Randomly set some buses as active
                if (Math.random() > 0.7) {
                    busData[busId].status = "active";
                } else {
                    busData[busId].status = "inactive";
                }
            }
        ...
        function updateBusMarkers() {
            for (const busId in busData) {
                ...
                if (markers[busId]) {
                    ...
                    markers[busId].setLatLng([bus.lat, bus.lng]);
                    
                    // Update icon based on status
                    let iconHtml = '';
                    if (busId === selectedBusId && userRole === 'student') {
                        iconHtml = '<div class="tracked-bus-marker"><i class="fas fa-bus"></i></div>';
                    } else {
                        iconHtml = `<div class="bus-marker" style="background: ${getStatusColor(bus.status)};"><i class="fas fa-bus"></i></div>`;
                    }
                    
                    markers[busId].setIcon(L.divIcon({
                        html: iconHtml,
                    ...
                    markers[busId].setPopupContent(`<b>Bus ${busId}</b><br>${bus.name}<br>Status: ${bus.status}<br>Last update: ${bus.lastUpdate.toLocaleTimeString()}`);
                } else {
                    // Create new marker
                    markers[busId] = L.marker([bus.lat, bus.lng], {
                        icon: L.divIcon({
                            html: `<div class="bus-marker" style="background: ${getStatusColor(bus.status)};"><i class="fas fa-bus"></i></div>`,
                            className: 'bus-marker-container',
                            iconSize: [24, 24],
                            iconAnchor: [12, 12]
                        })
                    ...
                    markers[busId].bindPopup(`<b>Bus ${busId}</b><br>${bus.name}<br>Status: ${bus.status}<br>Last update: ${bus.lastUpdate.toLocaleTimeString()}`);
                }
            }
        }
        ...
        function updateBusList() {
            ...
            for (const busId in busData) {
                ...
                busItem.innerHTML = `
                    ...
                    <div class="bus-info">
                        <h3>${bus.name}</h3>
                        <p>Last update: ${bus.lastUpdate.toLocaleTimeString()}</p>
                    </div>
        ...
        function updateStats() {
            ...
            for (const busId in busData) {
                totalBuses++;
                if (busData[busId].status === 'active') activeBuses++;
                if (busData[busId].status === 'ontime') onTime++;
                if (busData[busId].status === 'delayed') delayed++;
            }
        ...
        function filterBusList() {
            ...
            busItems.forEach(item => {
                const busName = item.querySelector('.bus-info h3').textContent.toLowerCase();
